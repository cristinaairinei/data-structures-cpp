...
void testAddIfNotPresent() {
	SortedMultiMap smm1(ascending); // create a SortedMultiMap with ascending order
	smm1.add(1, 10);
	smm1.add(2, 20);

	SortedMultiMap smm2(ascending);
	smm2.add(2, 200); // already in smm1
	smm2.add(3, 30);  // new key

	int added = smm1.addIfNotPresent(smm2);
	assert(added == 1); // only key 3 should be added

	assert(smm1.search(3).size() == 1); // key 3 should be present in smm1
	cout << "testAddIfNotPresent passed!\n";
}
