...
void testPositionOfExtended() {
    cout << "Test positionOf" << endl;
    Matrix m(10, 10);
    m.modify(3, 5, 20);

    pair<int, int> pos = m.positionOf(20);
    assert(pos.first == 3 && pos.second == 5);

    pair<int, int> posNotFound = m.positionOf(50);
    assert(posNotFound.first == -1 && posNotFound.second == -1);
}
